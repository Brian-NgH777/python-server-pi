...
def thread_function(thead):
    if thead == 0 :
        ...
        # os.system()
        subprocess.Popen(cmd1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, shell=True)
    elif thead == 1 :
        # Popen(cmd2, shell=True, stdout=DEVNULL)
        #  os.spawnl(os.P_DETACH, cmd2)
        subprocess.Popen(cmd2, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, shell=True)
        # os.system(cmd2)
    # elif thead == 2 :
